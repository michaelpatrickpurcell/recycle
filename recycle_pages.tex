...
\setkomafont{paragraph}{\setmainfont{Tex Gyre Schola}\normalsize\bfseries}
...
\tikzset{pics/textcard/.style n args={2}{
code={
	\node[draw, ultra thick, #1, minimum height=0.42in, minimum width=0.3in, rounded corners=1mm] at (0,0) {};
	\pic[scale=0.5, transform shape] at (0,0) {#2};
}}}
...
\item Whoever collected the most moon icons during the round will score their face-down sets during the colour-scoring phase (see \textbf{Colour Scoring}) instead of their face-up sets.% In the case of a tie, whoever collected the lowest-numbered moon card will receive this bonus.
\item Record how many sun icons you collect during each round. At the end of the game, if you collected the most total sun icons across all three rounds, your final score is set to zero.% In the case of a tie, whoever collected the lowest-numbered sun card during the final round of the game will receive this penalty.
\end{enumerate}
...
\begin{center}
\tikz{\pic {textcard={Red}{textstar}};} \tikz{\pic {textcard={Red}{textmoon}};} \tikz{\pic {textcard={Red}{textmoon}};} \tikz{\pic {textcard={Red}{textsun}};} \hfill \tikz{\pic {textcard={LimeGreen}{textstar}};} \tikz{\pic {textcard={LimeGreen}{textstar}};} \tikz{\pic {textcard={LimeGreen}{textmoon}};} \hfill \tikz{\pic {textcard={RoyalBlue}{textmoon}};} \tikz{\pic {textcard={RoyalBlue}{textsun}};}
\end{center}
...
%\section*{Cooperative Variant}
%When playing cooperatively, your goal is to ensure that a dummy player wins the game.
%
%Play the game as you would if playing competitively. The only change is that whoever collected the most total sun icons at the end of the game loses ten points rather than having their score set to zero.
%
%If a dummy player has the most points at the end of the game, you win. If not, you lose. Your team score is equal to the lowest player's score.
%
%To make the game more challenging, you can reduce the reward for collecting star icons to be just one point per icon.
